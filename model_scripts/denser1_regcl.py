...
def train_model():
    ds = RegClsGen(1000)
    vs = RegClsGen(1000)
    filename='/scratch/daint/jmahapat/rRegression_Classification/denser1_regcl'
    model = loadmodel(filename)
    model.compile(loss=['binary_crossentropy','mse'],optimizer='sgd')
    check = Checkpoint_RegCls(filepath=filename, verbose=0)
    early = EarlyStopping(monitor='val_loss', patience=10, verbose=0, mode='auto')
    print('%s has started training'%filename)
    hist = model.fit_generator(ds.train(modeltype=1), samples_per_epoch=50000, nb_epoch=1000, validation_data= vs.validation(modeltype=1), nb_val_samples=50000, verbose=0, callbacks=[check,early])
